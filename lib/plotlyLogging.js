...
function collectAuthentication() {
  var i;

  acctRef = {};
  for (i = 0; i < config.authentication.length; i += 1) {
    if (acctRef[config.authentication[i].userName] !== undefined) {
      throw new Error('Plotly username "' + config.authentication[i].userName +
        '" configuration is not unique');
    }
    acctRef[config.authentication[i].userName] = {
      index: i,
      nextToken: 0
    };
  }
...
function initializePlotly(callback, components) {
  var traceTemplate, plotTemplate, plotCount, plotData,
    targetTemplate, plotTarget, i, idx;
  ...
  for (i = 0; i < plotCount; i += 1) {
    ...
    idx = acctRef[plotTarget.userName].index;
    plotBlock[i] = {
      plotly: new Plotly(plotTarget.userName, config.authentication[idx].apiKey)
    };

    plotData = buildPlotData(i, plotTemplate, idx, components);
 ...
 * @param {Integer} configIndex Index to authentication configuration for plot
 * @param {Array} logable       string component ids available to log events for
 * @return {Array of Object}
 */
buildPlotData = function (instance, template, configIndex, logable) {
  var data, traceSource, i, trace, cfg, ref;
  data = [];
  cfg = config.authentication[configIndex];
  ref = acctRef[cfg.userName];

  plotBlock[instance].stream = [];
  traceSource = config.plots[instance].plotData;
  for (i = 0; i < traceSource.length; i += 1) {
    ...
    if (!logable.includes(trace.component)) {
      ...
      if (ref.nextToken >= config.authentication[configIndex].tokens.length) {
      ...
      ref.nextToken += 1;
      console.log(trace.stream.token);
