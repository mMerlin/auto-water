...
var acctRef, plotBlock, plotlyInstanceReady, deepCopyStandardObject,
  buildPlotData, addSensorListeners, plotlyCommunicationEstablished, isPlotReady;
var initDone = false;
...
var message = {
  ...
  badRecord: 'Invalid event recording configuration for "{id}" from plot' +
    ' {plot}, stream {stream}',
  badEvent: '"{event}" is not recognized logging event for "{context}" "{id}"',
  notSensor: 'The passed argument is not a Sensor component',
  notBoard: 'The specified object is not a component or board'
};
...
function doHeartbeat() {
  ...
  for (i = 0; i < plotBlock.length; i += 1) {
    for (j = 0; j < plotBlock[i].stream.length; j += 1) {
 ...
 * @param {Date} xValue       data point X coordinate (time point)
 * @param {number} xValue     data point Y coordinate
 * @return {undefined}
 */
function streamDataPoint(stream, xValue, yValue) {
  //IDEA: turn this into a queue entry instead of write, then use heartbeat
  // to flush queue when no new entry for delay time
  // Also flush regular data recording entries () when no state changes
  // recent/pending
  stream.stream.write(JSON.stringify({
    x: getDateString(xValue),
    y: yValue
  }) + '\n');
}// ./function streamDataPoint(trace, xValue, yValue)
  ...
  for (i = 0; i < plotBlock.length; i += 1) {
    ...
    for (j = 0; j < strm.length; j += 1) {
      if (strm[j].component === this.id) {
        // && strm[j].log === 'data'
        streamDataPoint(strm[j], Date.now(), value);
      }
    }
  }
}
  ...
  for (i = 0; i < plotBlock.length; i += 1) {
    ...
    for (j = 0; j < strm.length; j += 1) {
      if (strm[j].component === component.id) {
        ...
        streamDataPoint(strm[j],
          Date.now(),
          strm[j].transform[newState] === undefined ?
              newState : strm[j].transform[newState],
          oldState);
      }
    }
  }
...
function collectAuthentication(cfg) {
  var i;

  acctRef = {};
  for (i = 0; i < cfg.length; i += 1) {
    if (acctRef[cfg[i].userName] !== undefined) {
      throw new Error('Plotly username "' + cfg[i].userName +
        '" configuration is not unique');
    }
    acctRef[cfg[i].userName] = {
      index: i,
      nextToken: 0
    };
  }
  // console.log('Account references:', acctRef);//DEBUG
}
 ...
 * @return {undefined}
 */
function initializePlotly(callback, components, loggingConfig) {
  /* jshint maxcomplexity: 9 */
  var traceTemplate, plotTemplate, plotCount, plotData,
    targetTemplate, plotTarget, i, auth;
  ...
  collectAuthentication(loggingConfig.authentication);

  // Create the base / global templates, even if they are empty
  targetTemplate = JSON.parse(JSON.stringify(loggingConfig.target || {}));
  traceTemplate = JSON.parse(JSON.stringify(loggingConfig.trace || {}));
  ...
  traceTemplate.extend(emptyTrace);

  plotBlock = [];
  plotlyInstanceReady = [];
  plotCount = loggingConfig.plots.length;
  for (i = 0; i < plotCount; i += 1) {
    // Build the plot specific trace template
    plotTemplate = JSON.parse(JSON.stringify(loggingConfig.plots[i].trace || {}));
    plotTemplate.extend(traceTemplate);// Get any global template properties
    // Build the plot specific target information
    plotTarget = JSON.parse(JSON.stringify(loggingConfig.plots[i].target || {}));
    plotTarget.extend(targetTemplate);
    if (acctRef[plotTarget.userName] === undefined) {
      throw new Error('No authentication information for plotly username "' +
        plotTarget.userName + '"');
    }
    auth = loggingConfig.authentication[acctRef[plotTarget.userName].index];
    plotBlock[i] = {
      plotly: new Plotly(plotTarget.userName, auth.apiKey)
    };

    plotData = buildPlotData(i, loggingConfig.plots, plotTemplate, auth,
      components);
 ...
 * @param {Array} plots         plot configurations for current user
 * @param {Object} template     Default trace property values
 * @param {Object} cfg          Authentication configuration for plot
 * @param {Array} logable       string component ids available to log events for
 * @return {Array of Object}
 */
buildPlotData = function (instance, plots, template, cfg, logable) {
  var data, traceSource, i, trace, ref;
  data = [];
  ref = acctRef[cfg.userName];

  plotBlock[instance].stream = [];
  traceSource = plots[instance].plotData;
  for (i = 0; i < traceSource.length; i += 1) {
    ...
    if (!logable.includes(trace.component)) {
      ...
      if (ref.nextToken >= cfg.tokens.length) {
        throw new Error('Not enough tokens available for account "' +
          cfg.userName + '" to log data for "' + trace.component +
          '" in plot ' + instance);
      }
      trace.stream.token = cfg.tokens[ref.nextToken];
      ref.nextToken += 1;
      ...
      // Provide direct lookup from the (unique) component id
      config[trace.component] = { plot: instance, stream: i };
      // Remove the non-plotly configuration properties from the trace
      delete trace.component;
      delete trace.transform;
      delete trace.record;
...
plotlyCommunicationEstablished = function (instance, total, callback, err, msg) {
  ...
  for (trace = 0; trace < block.stream.length; trace += 1) {
    block.stream[trace].stream = block.plotly.stream(block.stream[trace].token,
      streamFinished);
  }
...
isPlotReady = function (err, msg) {
  ...
  console.log('\nPlot being created at:', msg.url);//report
  return true;
...
function addSensorEventHandler(component) {
  ...
  if (!(component instanceof events.EventEmitter)) {
    throw new Error(message.notSensor);
  }

  if (!config[component.id]) {
    // Just ignore any valid (sensor) component that is not configured for
    // logging.  Nothing more to do here.
    return;
  }

  addSensorListeners(component);

  context = Object.keys(loggedComponents).length;//DEBUG
  // Do not record the component id until after adding the listener(s):
  // Invalid components will fail on the add, so the id does not polute the data
  loggedComponents[component.id] = true;
}

addSensorListeners = function (component) {
  /* jshint maxcomplexity: 10 */
  var lookup, eventConfig, record, stream, i;
  lookup = config[component.id];
  eventConfig = plotBlock[lookup.plot].stream[lookup.stream];
  record = eventConfig.record;
  if (!(record === undefined || typeof record === 'string' ||
      Array.isArray(record))) {
    throw new Error(message.badRecord.
      supplant({ id: component.id, plot: lookup.plot, stream: lookup.stream }));
  }
  if (record === undefined || record === 'default') {
    record = 'data';
  }
  stream = typeof record === 'string' ?
  ...
  for (i = 0; i < stream.length; i += 1) {
    if (stream[i] === 'data') {
      ...
      throw new Error(message.badEvent.
        supplant({ event: stream[i], context: 'Sensor', id: component.id }));
    }
  }
};
...
function addControlEventHandler(component) {
  ...
  if (typeof emitter.loop !== 'function') {
    throw new Error(message.notBoard);
  }

  // Save reference to board, not just (boolean) that it has been seen
  context = Object.keys(loggedBoards).length;//DEBUG
